...
custom_shapes <- c(
  "Wasm" = "../thesis/assets/wasm.png",
  "Docker" = "../thesis/assets/docker.png"
)
...
load_data_and_plot_scatter <- function(file_path) {
  ...
  docker_img <- rasterGrob(readPNG("../thesis/assets/docker.png"))
  wasm_img <- rasterGrob(readPNG("../thesis/assets/wasm.png"))
  ...
  data <- data_raw %>%
    ...
    select(startup_time, energy_consumption_wh, func_type, func_name, input)

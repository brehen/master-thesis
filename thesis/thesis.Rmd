...
(*PF*)\todo{Explain wtf PF is.}, while assuming the voltage (*U*) is a constant
240V. Using these numbers we will calculate the power with the following
equation: \[P = U \times I \times PF\] Two sets of power consumption data will
be collected.
  ...
  invoking a function. This will be an estimation based on energy readings read
  during the lifetime of each function, and getting the average of these.

  - Isolated Average Power: The average power consumption attributed specificall
  to function execution, calculated by subtracting the baseline power from the
  average power. \(P_{\text{iso}} = P_{\text{total}} - P_{\text{baseline}}\)
...
consumption values will be derived:
  ...
  - Isolated Energy consumption: The energy in *Wh* consumed by the function,
  with the baseline power subtracted from the base *W* used to calculate this.
...
To ensure the reliability and validity of the results, the following measures
will be taken: controlled experimentation will be used to minimize the influence
of external factors; the experiments will be repeared multiple times to ensure
consistency of the results; the data will be analyzed using statistical
techniques to identify any patterns or trends; and the results will be compared
to existing research in the field, explored in \Cref{sect:related-work}.
...
Functions deployed to Nebula will either be in the form of a \ac{Wasm} module
binary file, or as a Docker image that is uploaded to the server and loaded into
the servers image repository. The packaging process will involve compiling the
source code to \ac{Wasm} module binaries, or creating a Docker image with the
use of a Docker file that describes its layers.

Our server will provide a way to let users upload their functions to the system.
The deployment process will handle the necessary steps to make the functions
available for invocation, such as storing the \ac{Wasm} modules or Docker image,
and preparing them for invocation. \todo{rephrase this mess}
...
with over HTTP in a REST-ful manner. In other word, we are going to develop a
...
The Nebula prototype will be deployed on two different environments to evaluate
...
which best matches our desired outcome. \todo[inline=true]{I ended up with Gude
...
the server through the REST API, and interface with the power supply over a
chosen protocol. 
...
scenarios. The Raspberry Pi setup will additionally give us insights into the
energy consumption of our functions, while the virtual machine will represent a
cloud-native deployment environment, with a more powerful CPU and more memory,
like we would expect of our cloud servers.
...
are included in the returned `FunctionResult` struct. As we will see in
\Cref{chap:results}, our \ac{Wasm} modules startup 
...
stdout processes. 
2. Write input to the stdin, which the function that is packaged inside our
   container is waiting on.
3. Wait for function inside the Docker container to execute and write out the
   result as stdout.
4. Parse output from the Docker container with a helper function, which comes in
...
are going to need some functions that we can deploy and run on Nebula. For thatj
...
Once inside the \ac{NREC} platform, we can launch an instance of our virtual machine,
following their guide\todo{add guide?} for doing so. For our experiments we
chose the pre-configured flavor of `m1.medium`, which gives us 1 virtual CPU, 4GB
of RAM and 20GB storage space. The virtual machine was built with the base image
of Debian 12. After this is set up, we can ssh into the server with a ssh-key we
...
existing tool. Because of this, we will develop a custom benchmark utility in 
Rust, which will let us collect the data we're interested in.
...
function results on our computer. We also need a function for measuring power
measurements while performing our POST requests to Nebula on our Raspberry Pi.
...
testing, that will give us a clear picture on how our prototype performs when
running functions as \ac{Wasm} modules or Docker containers.
...
parallelism. These factors help 
...
Now that we have achieved the first goal of our thesis, to develop a prototype
cloud computing platform for the \ac{FaaS} paradigm, we will explore the
remaining goals and attempt to answer them in this part. Based on the prototype
we implemented in \Cref{chap:implement_nebula} and the supporting benchmark
utility we also developed, we have experimented on Nebula, and are now ready to
examine the results.

\section{Methodology recap}

For our experiments we implemented a series of benchmark functions that were to
run on both our Raspberry Pi and a standard virtual machine on \ac{NREC} to mimic different
operational environments. Each function was tested as both a \ac{Wasm} module
and Docker container, and we aimed to capture a broad range of data under
controlled condotions. The specific metrics we aimed to measure were:

- Startup Latency (Cold start): Time from the function is invoked until it
begins executing.
- Total Runtime: Duration of the entire functions lifetime, from start until it
returns the result.
- Power consumption: The amount of *W* that we measured on our Raspberry Pi
while under load of our function executions. A product of the measured current
and power factor, where we assume a constant voltage of 240V.

As a result of our benchmarks measuring total runtime and power consumption, we
also have the benefit of deriving an estimation of how energy our function
exections consumed during its lifetime. Energy consumption (*E*) is measured in Wh,
a product of \(E = P \times t\).

We automated the data collection through our benchmarking utility implemented in
\Cref{sect:implement_benchmark} to ensure accuracy and repeatabiity, which
proved invaluable while gathering data for this report.

Important to note for this chapter, is that a lot of the data the differences
between our function invocations as \ac{Wasm} modules and Docker containers are
so extreme that it is hard to properly visualize them next to each other in the
same graph. Therefore, where applicable, the data will be split into \ac{Wasm}
and Docker for our data sets, to be able to examine the data more closely. 

\section{Startup and Runtimes}


```{r, avg_times_nrec_rpi, echo=F, warning=F, dev = "cairo_pdf", fig.height=7, fig.cap="Average Startup and Runtime on our setups"}
...
ggplot(averaged_data, aes(x = interaction(func_name, source_func_type), y = value, fill = Metric)) +
  ...
  # ggtitle("Average Startup and Runtimes on both setups") +
...
In \Cref{avg_times_nrec_rpi} above we can see the startup and runtimes of
our function invocations across both our Nrec and Raspberry Pi deployment
targets, across our 4 benchmark functions, split by Docker or \ac{Wasm}
invocations. As we will present in a later graph, the fibonacci and prime number
functions data set include input that towards the higher end ramps up the
runtime exponentially. These extremes have been excluded from the sample in
this graph, so we can closer examine the average startup and runtimes we could
expect. (Specifically, input above 1500 for Prime numbers, and above 30 for
Fibonacci)  
...
```{r, echo=F, warning=F}
wasm_nrec_data <- nrec_data %>%
  filter(func_type == "Wasm")

ggplot(wasm_nrec_data, aes(x = input, y = runtime, color = func_type)) +
  geom_point(alpha = 0.1) +
  ggtitle("Runtimes in ms on NREC virtual machine") +
...
wasm_rpi_data <- rpi_data %>%
  filter(func_type == "Wasm")

ggplot(wasm_rpi_data, aes(x = input, y = runtime, color = func_type)) +
  geom_point(alpha = 0.1) + # Adjust transparency with alpha
  ggtitle("Runtimes in ms on RPI") +
  scale_color_manual(values = c("Docker" = "#089CEC", "Wasm" = "#6B54F1")) +
  geom_smooth(aes(label = func_type),
    fill = "#69b3a2",
    method = "loess", formula = y ~ x,
...
print_power_graph <- function(data_set) {
  ggplot(data_set, aes(x = input, y = average_power, color = func_type)) +
    geom_point(alpha = 0.1) + # Adjust transparency with alpha
    ggtitle("Power consumption on Raspberry Pi") +
    scale_color_manual(values = c("Docker" = "#089CEC", "Wasm" = "#6B54F1")) +
    geom_smooth(aes(label = func_type),
      fill = "#69b3a2",
      method = "loess", formula = y ~ x,
      size = 3, linewidth = 2, boxlinewidth = 0.9,
      se = TRUE
    ) +
    labs(
      x = "Input", y = "Power in Watts (W)", color = "Function Type",
    ) +
    facet_wrap(~func_name, scales = "free") +
    theme_classic() +
    theme(legend.position = "bottom")
...
print_energy_graph <- function(data_set) {
  ggplot(data_set, aes(x = input, y = energy_consumption_wh, color = func_type)) +
    geom_point(alpha = 0.1) + # Adjust transparency with alpha
    ggtitle("Energy consumption (μWh) on Raspberry Pi") +
    scale_color_manual(values = c("Docker" = "#089CEC", "Wasm" = "#6B54F1")) +
    geom_smooth(aes(label = func_type),
      fill = "#69b3a2",
      method = "loess", formula = y ~ x,
    ...
    labs(
      x = "Input", y = "Energy in micro Watt-hours (μWh)", color = "Function Type",
    ) +
    facet_wrap(~func_name, scales = "free") +
    theme_classic() +
    theme(legend.position = "bottom")
...
```{r, fib_power, echo=F, warning=F, dev = "cairo_pdf", fig.cap="Average Power load during function execution"}
fib_rip_data <- rpi_data %>%
  filter(func_name == "fibonacci-recursive")

print_power_graph(fib_rip_data)
```

```{r, fib_energy, echo=F, warning=F, dev = "cairo_pdf", fig.cap="so beautiful"}
print_energy_graph(fib_rip_data)
```

```{r, fact_power, echo=F, warning=F, dev = "cairo_pdf", fig.cap="Average Power load during function execution"}
fact_rip_data <- rpi_data %>%
  filter(func_name == "factorial")

print_power_graph(fact_rip_data)
```

```{r, fact_energy, echo=F, warning=F, dev = "cairo_pdf", fig.cap="so beautiful"}
print_energy_graph(fact_rip_data)
```

```{r, exp_power, echo=F, warning=F, dev = "cairo_pdf", fig.cap="Average Power load during function execution"}
exp_rip_data <- rpi_data %>%
  filter(func_name == "exponential")

print_power_graph(exp_rip_data)
```

```{r, exp_energy, echo=F, warning=F, dev = "cairo_pdf", fig.cap="so beautiful"}
print_energy_graph(exp_rip_data)
```
```{r, prime_power, echo=F, warning=F, dev = "cairo_pdf", fig.cap="Average Power load during function execution"}
prime_rip_data <- rpi_data %>%
  filter(func_name == "prime-number")

print_power_graph(prime_rip_data)
```

```{r, prime_energy, echo=F, warning=F, dev = "cairo_pdf", fig.cap="so beautiful"}
print_energy_graph(prime_rip_data)
```

\newpage

```{r, runtime_prime_number, echo=F, warning=F, dev = "cairo_pdf"}
combined_data <- bind_rows(
  nrec_data %>% mutate(source_func_name = paste(source, func_name, sep = "_")),
  rpi_data %>% mutate(source_func_name = paste(source, func_name, sep = "_")),
)

power_rip_data <- combined_data %>%
  filter(func_name == "fibonacci-recursive") %>%
  filter(func_type == "Wasm")

avg_srtup_data <- power_rip_data %>%
  # filter(func_type == "Wasm") %>%
  filter(!(func_name == "fibonacci-recursive" & input > 31)) %>%
  filter(!(func_name == "prime-number" & input > 1500))
...
averaged_data <- avg_srtup_data %>%
  group_by(source_func_name, input) %>%
  summarize(
    avg_startup_time = round(mean(startup_time), digits = 2),
    avg_runtime = round(mean(runtime), digits = 2),
    func_name = func_name
...
# Creating a ggplot with stacked bar chart
ggplot(averaged_data, aes(x = input, y = value, fill = metric)) +
  geom_line() +
  facet_wrap(~source_func_name, scales = "free", ncol = 2) +
  labs(x = "Function Name", y = "Time in milliseconds") +
  theme_ipsum(base_family = "Avenir", base_size = 12, axis_text_size = 10, plot_title_size = 10, axis_title_size = 15, strip_text_size = 15) +
  theme(
    legend.position = "bottom", axis.title.x = element_blank(),

...
\todo{Write WASI section}

\subsection{WebAssembly runtimes}

\ac{Wasm} by itself can't run by itself. It needs to be interpreted by a runtime
and ran by it. This is where Wasm runtimes like Wasmtime and Wasmer comes in.

\todo{Write more wasmtime and wasmer}
...
\todo{Maybe this section should be moved to the discussion/results page and be
...
1. Raspberry Pi: A small \ac{SBCs} with an ARM64 architecture that can run
...
\todo{elaborate that the functions chosen are to be Pure, meaning that Nebula in

...
\ac{Wasm} modules and traditional Docker containers. This chapter discusses the
key findings and their significance and relates them to existing research in
this domain.

\section{Startup Latency and Runtime Performance}

One of the most noticable findings from our experiments is the substantial
difference in startup latency between \ac{Wasm} modules and Docker containers.
On the virtual machine setup, \ac{Wasm}-based function invocations exhibited
startup times approximately 300-400 times faster than their Docker counterparts.
This discrepancy was also observed on the resource-constrained Raspberry Pi,
albeit with a bit smaller gap, where \ac{Wasm} startups were around 240 times
quicker than Docker containers. 

...
The launch of AWS Amazon S3 cloud service in March 2006, followed by Elastic
...
Switch at a given interval and publish the values on a MQTT-topic. This is in
turn picked up by the MQTT broker and published to its subscribers. This switch
...
An alternative to reading energy measurements through a pub-sub model with MQTT,
...
HTTPS, SNMP, Telnet, MQTT and Modbus TCP \citep{gmbhExpertPowerControl2023}. In
...
be using NREC \todo{add citation/link}, a IaaS platform available to students,
...
2. Virtual Machine: A Debian-based VM running on an \ac{IaaS} platform (e.g.,
   NREC), to measure performance relative to how a typical cloud native
...
hosted on an IaaS platform (e.g., NREC) for a more typical cloud setup. \\
...
virtual machine running Debian on NREC. While a Raspberry Pi 4 will serve as our
...
+ Modbus tcp, should I just cut out Smart switch and MQTT/Zwave from the thesis,
...
to offer a \ac{FaaS} powered by \ac{WebAssembly} modules. They have built a
...
\citet{malmgrenGettingDataOut2022} suggested in their helpful article, using
...
Docker container to run on. For our NREC deployment, we will target linux/amd64
...
Raspberry Pi and the Debian-based virtual machine on NREC.
...
\subsection{Virtual Machine setup (NREC)}
...
Once inside the NREC platform, we can launch an instance of our virtual machine,
...
\centering
  ...
  \caption{Our virtual machine configured on NREC.}
  \label{fig:build_deploy_function}
\end{figure}
...
source code onto the device, and building it from there. For the NREC
...
Raspberry Pi and virtual machine on NREC, we can start implementing the
...
over MQTT. This setup had one major flaw; energy reporting through MQTT could
...
communcation method from MQTT to Modbus TCP, we were able to perform energy
...
the choice of the Rust programming language was deliberate and brought inherent
benefits in terms of code reliability and correctness, ensuring the validity and
rigidity of the prototype.

\subsection{Rust's Compiler Guarantees}

As mentioned in \Cref{sect:tech_stack}, the selection of Rust was driven by its strong
relationship with WebAssembly and its suitability for building efficient and
scalable applications. Furthermore, Rust's strict compiler checks and emphasis
on memory safety, concurrency, and parallelism provided a solid foundation for
developing a robust and reliable system.
...
Moreover, Rust's ownership model and borrowing rules, ensure that memory is
managed safely and efficiently, without the need for manual memory management or
a garbage collector. This reduces the risk of memory-related bugs, which are
notoriously difficult to detect and fix. 

An exception to these guarantees can be found in the case of deserializing our
\ac{Wasm} modules as discussed in \Cref{sect:6-challenge_insights}. Here we opt
out of Rust's compiler guarantees by telling Rust that we know better in this
instance, and read our \ac{Wasm} binary files as byte code. This is a potential
risk, in case someone manages to sneak in harmful binaries into the web server,
but did not pose an issue for our experiments.

\subsection{Leveraging the Rust Ecosystem and Best Practices}

Throughout the development of Nebula, idiomatic Rust practices and best
practices from the Rust community were followed. This included adhering to
guidelines such as favoring composition over inheritance, embracing functional
programming patterns where applicable, and utilizing Rust's robust ecosystem of
crates (libraries), as highlighted in \Cref{sect:tech_stack}.

By aligning with these practices and leveraging well-tested, community-driven
solutions, the codebase remains maintainable, extensible, and aligned with the
collective wisdom of the Rust community, reducing the likelihood of subtle bugs
or design flaws.

\subsection{Rigorous Testing and Benchmarking}

While formal unit testing was not a primary focus, the code underwent rigorous
manual testing and code review processes. As detailed in Section 6.6, each
component of Nebula was thoroughly tested with various input scenarios, edge
cases, and stress tests to ensure its correctness and robustness.

Additionally, the benchmarking suite itself, as described in
\Cref{sect:impl_bench}, served as a form of integration testing, exercising the
entire system end-to-end and validating the correctness of the results. 

The choice of Rust, combined with idiomatic coding practices, adherence to best
practices, and extensive testing through the benchmarking suite, provided a
strong foundation for the validity and rigidity of the Nebula prototype,
ensuring reliable and robust performance during the experiments and evaluation.

...
The code presented in this chapter is shortened down to focus on the important
parts that we will use for our experiments. There is also an interactive GUI
...
manual testing, rather than a feature that supports this thesis. The code for
this GUI will not be part of the code examples shown, but can be found in its
entirity in the projects repository. \todo[inline=true]{Will I hand in the
repository as part of the hand in?}

\section{Choosing a tech stack}

The programming language Rust was chosen to build our prototype. It has a great
story with \ac{Wasm}, with the same company, Mozilla, having developed both languages. 
Rust is a <describe rust here>. There also exist a Rust crates (Rust's naming
convention for libraries) for most of the \ac{Wasm} runtimes that we could
choose to employ for our application. Using one of these crates will allow us to
ship the entire \ac{Wasm} runtime required for running our \ac{Wasm} modules as
part of the web server itself, meaning that we won't have to install a separate
runtime on our server as part of our prototype deployment.

For our \ac{Wasm} runtime, we're going to opt for
Wasmtime^[https://wasmtime.dev/], as it offers a relatively easy way to install
and package our runtime with our application. <enter more reasons>

For our Docker environment, we will have to install Docker desktop on both our
development computer, and on each deployment target. Docker desktop has support
for both Debian on ARM64 on our Raspberry Pi, and on Debian on our virtual
machine. We will write a Rust function that spins up a command for running our
docker image during execution.

\section{Building a web server}

To let our users interact with our application, we need to spin up an
application that they can interface against. For this we are going to build a
web server that exposes a REST API, and for this we will use Actix
Web^[https://actix.rs/], a web framework that builds upon the asynchronous Rust
runtime Tokio, which we will use for running multiple processes at once.  

The code below describes how the main entry point for our web server is built.
We define an asynchronous main function, using a Rust macro `[#tokio::main}]`.
Then we define a router with two end points, one for calling functions, and one
for retrieving stored function results.
...
use nebula_server::{
    api::call_function::call_function_headless, 
...
async fn main() {
  ...
  let router = Router::new()
      ...
      .route("/invoke_function", post(call_function_headless))

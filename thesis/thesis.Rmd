...
nrec_data <- nrec_data_raw %>%
  ...
  select(startup_time, runtime, total_runtime, func_type, func_name, input)


rpi_data_raw <- fromJSON("./assets/metrics/rpi_energy_data.json", flatten = TRUE)
...
rpi_data <- rpi_data_raw %>%
  ...
  select(startup_time, runtime, total_runtime, energy_consumption_wh, func_type, func_name, input)
```
...
Now that we have ventured through planning our experimental research, to
designing a prototype and planning our benchmarkings, and finally implemented
and performed our benchmarkings, it is time to look at explore the data we have
gathered through our benchmarkings.

\section{Methodology recap}

For our experiments we implemented a series of benchmark functions that were to
run on both our Raspberry Pi and a standard virtual machine on NREC to mimic different
...
The data collection was automated through our benchmarking utility written in
Rust to ensure accuracy and repeatabiity, which proved invaluable while
...
```{r, echo=F, warning=F}
gg <- ggplot(nrec_data, aes(x = input, y = total_runtime, color = func_type)) +
  geom_point(alpha = 0.4) + # Adjust transparency with alpha
  ggtitle("Startup times in ms on NREC virtual machine") +
...
gg <- ggplot(averaged_data, aes(x = interaction(func_name, func_type), y = value, fill = Metric)) +
  geom_col(position = "stack") +
  ggtitle("Average Startup and Runtimes on NREC virtual machine") +

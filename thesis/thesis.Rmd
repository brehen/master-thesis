---
title: Nebula\:\ Performance and Energy Efficiency in Serverless Computing
...
header-includes: 
  ...
  - \titleformat{\part}
    {\headingfont\huge\bfseries\centering\vfill} 
    {\partname\ \thepart} 
    {12pt} 
    {\centering} 
    [\vfill] 
  ...
  - \renewcommand{\cfttoctitlefont}{\hfill\headingfont\Large\bfseries}
  - \renewcommand{\cftaftertoctitle}{\hfill\mbox{}\par}
  - \renewcommand{\cftpartfont}{\headingfont\large}
  - \renewcommand{\cftpartpagefont}{\headingfont\large}
  - \renewcommand{\cftchapfont}{\headingfont\mdseries}
  - \renewcommand{\cftchappagefont}{\headingfont\mdseries}
  ...
  - \renewcommand{\cftloftitlefont}{\hfill\headingfont\Large\bfseries}
  - \renewcommand{\cftafterloftitle}{\hfill\mbox{}\par}
  - \renewcommand{\cftfigfont}{\headingfont}
  - \renewcommand{\cftfigpagefont}{\headingfont}
  - \renewcommand{\cftlottitlefont}{\hfill\headingfont\Large\bfseries}
  - \renewcommand{\cftafterlottitle}{\hfill\mbox{}\par}
  - \renewcommand{\cfttabfont}{\headingfont}
  - \renewcommand{\cfttabpagefont}{\headingfont}
...
This thesis explores WebAssembly with \ac{WASI} as an innovative choice for
deploying functions to the cloud, through developing a prototype \ac{FaaS}
platform named Nebula. This platform will run functions compiled to WebAssembly,
originally designed for high-performance tasks in web browsers, which coupled
with \ac{WASI}, allows us to give WebAssembly programs access to the underlying
system. This holds potential for a more efficient way to package and deploy
functions, potentially reducing the startup latency and the overhead associated
with traditional serverless platforms. WebAssembly and \ac{WASI} offers a
pathway, where the demands of today is met, while reducing the carbon footprint
for cloud applications.

\section{Problem Statement}
\label{sect:problems}

In this thesis, we are interested in exploring the potential of WebAssembly and
\ac{WASI} as an energy-efficient alternative for deploying functions in a
serverless computing environment. The primary focus is to investigate whether
WebAssembly-based functions can offer improved performance and reduced energy
consumption compared to traditional containerized functions.

Based on this, we can define these goals that we want to achieve with this
thesis:

- Develop a prototype cloud computing platform for the \ac{FaaS} paradigm.
- Use this prototype to compare the performance of WebAssembly-based functions to containerized
functions in terms of startup latency and execution time
- Find out if WebAssembly-based functions consume less energy than containerized
functions when executing equivalent tasks

By achieving these goals this thesis seeks to shed light on the feasibility and
implications of adopting WebAssembly and \ac{WASI} for a greener cloud.

\section{Outline}

The thesis is structured to guide the reader through the background, design,
implementation, and analysis of the Nebula project. The key aspects covered
include:

- The evolution of cloud computing and the emergence of WebAssembly as a
promising technology for serverless computing.

- The design and implementation of the Nebula prototype, highlighting the
integration of WebAssembly and WASI, using the Rust programming language.

- The experimental setup and methodology employed to compare the performance and
energy consumption of WebAssembly-based functions against containerized functions.

- An in-depth analysis and discussion of the experimental results, addressing
the implications for energy efficiency in serverless computing.

- Concluding remarks and suggestions for future work, exploring the potential
...
to containerization, the third wave encompasses utilizing WebAssembly and
...
\Cref{fig:headbook} for an example of this kind of architecture)
...
efficient way to package and deploy our programs? WebAssembly and \ac{WASI}, as
...
C, C++, Go, and build tiny modules that can run on a WebAssembly runtime. 

These runtimes can run on pretty much any architecture with ease,
the resulting binary size are quite small, and the performance is near-native.
These perks combined with the potential for reduced overhead, smaller image
sizes, and faster startup times make WebAssembly and \ac{WASI} a promising
candidate for the third wave of cloud compute with a lower impact on the
environment.

Recent advancement in the space has also seen the development of the
WebAssembly component model^[https://component-model.bytecodealliance.org/].
With this model we will be able to piece together our tiny WebAssembly modules
compiled from multiple source languages into a larger puzzle where we can build
polyglot applications using multiple languages, as illustrated in
\Cref{fig:wasm-wave}. 
...
and now WebAssembly with \ac{WASI} - represent the industry's pursuit of more
...
exciting to see how WebAssembly and \ac{WASI} can be leveraged in this third wave and
...
data center will consume compared to the entire energy consumption of Norway.
...
It is important to note that while Google is committed to using renewable
energy, their presence in the area does not necessarily lead to an increase in
renewable energy infrastructure. If Google purchases all the available green
energy in the region, it may result in other consumers relying more on
non-renewable energy sources \citep{sundtConsumersWillingnessPay2015}. 
...
It is crucial to acknowledge that the increasing demand for data centers also
leads to a higher overall power demand. This increased demand for energy can
result in more natural habitats being destroyed to accommodate the necessary
power infrastructure. The construction of new data centers and the associated
energy infrastructure can have a significant impact on the environment and local
ecosystems \citep{bolognaDeforestationWorldPopulation2020}.
...
physical servers, cloud providers can improve resource utilization and
reduce the energy consumption of their data centers.
\citep{beloglazovEnergyawareResourceAllocation2012} 

To summarize, in the context of the growing energy consumption of data centers
and its environmental impact, major cloud providers such as Google, Microsoft,
and Amazon have made significant commitments to renewable energy adoption and
sustainability. However, it is important to acknowledge that reducing energy
consumption is the most sustainable approach. Researchers and industry experts
have emphasized the importance of prioritizing energy efficiency, optimizing
resource utilization, and adopting innovative solutions like submerged servers
and power management techniques to minimize the environmental footprint of cloud
computing \{shehabiUnitedStatesData2016,mytton2020,danilakCouncilPostWhy2017}.
The European Climate, Infrastructure and Environment Executive Agency
underscores this point, stating, "The greenest and most ethical energy is the
energy we don't consume" \citep{cappellettiGreenestMostEthical2022}.
...
2. Self-healing and monitoring: Orchestration tools can monitor the health of
   containers and automatically restart or reschedule them in case of
failures^[https://kubernetes.io/docs/concepts/overview/].
...
architectures^[https://kubernetes.io/docs/concepts/services-networking/].
...
alternative approaches, such as using WebAssembly and \{WASI}. As Solomon Hykes,
...
This statement highlights the potential of WebAssembly and \ac{WASI} to face the
...
WebAssembly, commonly abbreviated as Wasm, is a modern binary instruction format
...
introduces the project that \ac{Wasm} evolved from - *asm.js* - and illustrate
how \ac{Wasm} lets developers write programs in a high-level language and
...
develop \ac{Wasm}, launching the first public version in 2017. \ac{Wasm} is
...
development of the WebAssembly System Interface^[https://wasi.dev/]. \ac{WASI}
is a modular system interface that provides a standardized set of functions for
interacting with the host operating system, enabling \ac{Wasm} modules to run
...
modules to be portable across different platforms and environments. Its first
iteration aimed to be implement as many POXIS-like features as possible, but has
since extended beyond this and in their latest Preview 2, they have implemented
support for websockets and HTTP interfaces \citep{WASIPreview2README}.
...
\ac{Wasm} modules cannot run independently; they require a runtime environment
to interpret and execute them. Several \ac{Wasm} runtimes have been developed
to support the execution of \ac{Wasm} modules in different environments, such
...
execution environment for \ac{Wasm} modules, enabling them to run on a wide
...
compilation, which can further improve the performance of \ac{Wasm} modules.
...
interest in using \ac{Wasm} as an alternative to traditional containerization
...
metering and monitoring that supports communicating over TCP/IP. It has a server
that can measure energy, current, power factor, phase angle, voltage, and active
/ apparent / reactive power. To read these measurements, the PDU supports a
handful of interfaces, such as REST API, HTTPS, SNMP, Telnet, \ac{MQTT} and
Modbus TCP \citep{gmbhExpertPowerControl2023}. In \Cref{fig:gude-control} below,
a simplified setup with the Gude acting as the server for communicating over
Modbus TCP is shown.
...
has access to, typically 240V in Norway on AC. The current
...
factor measures between 0 to 1 and describes the amount of power consumed
spent on actual usage. For example, a power factor of 0.5 means that 50%
of the power consumed in a circuit goes to waste. Most data centers aim to get
as close to a power factor 1 as possible, to make most of the energy they pay
for not go to waste \citep{rasmussenImpactLeadingPower2018}.
...
The focus of this thesis is to build a prototype and perform experiments on
...
be using \ac{NREC}^[https://nrec.no/], an IaaS platform available to students,
...
computational workloads will be implemented and executed on both environments to
assess the suitability of the \ac{FaaS} prototype for diverse application
scenarios.
...
images. The selection of these functions is based on their ability to stress
different aspects of Nebula, including function invocation mechanisms, memory
management, computational intensity, and handling of large datasets.

The functions we will implement are:

1. *Fibonacci*: This function calculates the *nth* Fibonacci number in the
sequence, following the  formula: \(F_n = F_{n-1} + F_{n-2} \). The recursive
nature of the Fibonacci function involves a significant number of function calls
and memory accesses, making it a relevant benchmark for evaluating the overhead
and efficiency of function invocations and memory utilization in the \ac{FaaS}
prototype.
...
which are common in scientific computing and data analysis applications.
3. *Factorial*: This function calculates the sum of the factorial of *n*,
following the formula: \(n! = \prod_{k=1}^n k\). Similar to the Fibonacci
function, the factorial function involves recursive computations and can stress
the function invocation and memory management aspects of the \ac{FaaS} platform.
4. *Prime number*: This function finds the *nth* prime number using a
   brute-force approach, looping through a vast range of numbers (0 through
\(2^{64} - 1\) to check for primality. This benchmark stresses the CPU and
memory capabilities of Nebula by simulating computationally intensive workloads
involving large datasets and long-running calculations, commonly found in areas
like cryptogrpahy.

Choosing what functions to implement is not an easy task. While these functions
are relatively simple in nature, and software applications are commonly more
complex, these four functions still provide a well-rounded evaluation by
stressing various aspects of Nebula. With these functions we can test
computational intensity, memory management, and handling of large datasets.
The selection balances comprehensiveness with practicality, letting us perform a
thorough evaluation within a reasonable scope.
...
As we will discuss in \Cref{sect:energy_challenges}, we will opt for
using the Gude Expert Power Control 1105 for measuring our energy readings.
\Cref{tab:gude_config} below shows a sample of sensors we can read from this
device. To read from each sensor we need its address represented in hex values
as we will discuss in more detail \Cref{sect:measure_power}.
...
data <- data.frame(
  ...
  Inaccuracy = c("< 1.5%", "< 1.5%", "< 1%", "< 1.5%", "< 0.03%", "< 3%")
)
...
containers. As we can see in \Cref{tab:gude_config} there will be some measure
of inaccuracy involved with our experiments, with the most impactful one being
the power factor at < 3\%. We will collect multiple measurements for each
permutation of our function invocations which should help account for this
inaccuracy, but it is still important to keep in mind.
...
Based on the requirements outlined in \Cref{sect:nebula_req}, we will design a
...
or Docker containers, like illustrated in \Cref{fig:user-interaction}.
...
approriate protocol as we saw in \Cref{fig:function-execution-design}.
...
told the journey of how they ended up developing their cloud platform 
...
build. Both projects are open-sourced and available on Github, so we
can take a look at how they have implemented their services to run \ac{Wasm}
modules in a serverless environment.

In this chapter, the code snippets are presented in a condensed form, focusing
on the parts relevant to our experiments. There is also an interactive GUI built
with HTMX and Askama HTML templating which the user can use to manually interact
with each deployed function, but this aspect is more a novelty for manual
testing, rather than a feature that supports this thesis. A lot of the source
code will not be shown, we are going to focus on the code that lets us run
functions as either \ac{Wasm} modules or Docker containers. The souce code can
be found on Github at the public repository for this
project^[https://github.com/brehen/nebula/tree/v1.0], for closer inspection.
...
For our Docker environment, we installed Docker Engine on both our development
computer and each deployment target. Docker Engine supports Debian on ARM64 for
...
to be captured from stdout, like traditional command-line programs. As
...
`Module::deserialize` function. As Rust is a statically typed program, we 
must provide the types for our code ahead of time, before compilation, and even if
...
from our functions, and ideally we will not have to implement that individually for
...
but some manual testing revelead a negligble difference, which shouldn not have
...
Finally, we add a simple run script that the image invokes when we run our
Docker containers as described in \Cref{sect:execute_docker}, invoking the main
...
function. More on that in \Cref{sect:func_storage}.
...
\Cref{sect:6-challenge_insights}, we treat our functions as normal programs, which we do
...
sequence we are interested in, which we have implemented recursively, and pass
...
required to run our experiments are identical. First we must update the
packages on our Debian server, and then install Docker Engine, following their
official guide in their manual^[https://docs.docker.com/engine/install/debian/].
...
let us collect the data we are interested in.
...
performing the request is omitted from this example, but the suite of
functions and their corresponding inputs we are testing with can be seen here:
...
when running functions as \ac{Wasm} modules or Docker containers.
...
report on. In \Cref{tab:gude_config}, we extracted the most relevant sensors for
our measurements, its register address, and how accurate its reported units are. 
...
\(240V \times 24mA \times 0.55 = 3,168mW\). This product is what we are going to
collect data on. 
...
calculate the estimated *μWh* in energy consumption, with *t* in microseconds,
with the formula: \[P \times \frac{t}{3600}\] 
...
down to measuring every second instead.
...
are very difficult to detect and resolve. We can not guarantee that
...
\Cref{chap:implement} and the supporting benchmark utility, experiments
...
benchmarking utility developed in \Cref{sect:impl_bench}, proving
...
```{r, avg-efficiency-nrec-rpi, echo=F, warning=F, dev = "cairo_pdf", fig.height=6.5, fig.align = 'center', fig.cap="Mean startup and runtimes on both our setups."}
...
\Cref{fig:avg-efficiency-nrec-rpi} provides an overview of the average startup and
...
containers may appear similar in \Cref{fig:avg-efficiency-nrec-rpi}, but a keen eye
...
Based on this, our findings are summarized in \Cref{tab:nrec-avg,tab:rpi-avg}.
These tables shows the direct comparison of each metric we set out to measure
between \ac{Wasm} modules and Docker containers on our setups.
...
power load by the total execution time in microseconds for each function. In
\Cref{fig:avg-energy-consumption-rpi}, we can see the average energy consumption
that our Rasbperry Pi consumes during their execution. 
...
```{r, avg-energy-consumption-rpi, echo=F, warning=F, dev = "cairo_pdf", fig.height = 4, fig.align = 'center', fig.cap="Average energy consumption for our functions."}
...
```{r, exp-energy, echo=F, warning=F, dev = "cairo_pdf", fig.align = 'center', fig.cap="Energy consumption on Raspberry Pi during Exponential benchmark."}
exp_rip_data <- rpi_data %>%
  filter(func_name == "exponential")
...
```{r, prime-power, echo=F, warning=F, dev = "cairo_pdf", fig.height = 4, fig.align = 'center', fig.cap="Power load on Raspberry Pi during Prime Number benchmark."}
...
```{r, prime-energy, echo=F, warning=F, dev = "cairo_pdf", fig.height = 4, fig.align = 'center', fig.cap="Energy consumption on Raspberry Pi during Prime Number benchmark."}
...
research in this domain. As shown in \Cref{tab:gude_config}, there is a small
degree of inaccuracy in the collected and analyzed data. However, despite this,
our results still provide a clear and accurate representation of the performance
...
lot across invocations, we can see that a consistent startup time trend. 
...
On the virtual machine, \ac{Wasm} runtimes were about 271 times faster than
...
as \ac{Wasm} modules. The ratio of our runtimes across both \ac{Wasm} and Docker
...
Interestingly, and not something we set out to explore, but is still worth a
mention, is that our \ac{Wasm} modules were relatively small in size. Especially
compared to the same source code packaged as a Docker image and gzipped.
\Cref{tab:function_sizes} shows the storage size, which were mostly identical,
for each of our benchmark functions. Using the strip-tool of the wasm-tools
program reduces the size of our \ac{Wasm} by ten times, while still being able
to run on Wasmtime without issue.

```{r, echo=F, eval=T}
data <- data.frame(
  Metric = c("Docker", "Wasm", "Wasm (stripped)", "Wasm vs Docker", "Wasm (stripped) vs Docker"),
  `Storage Size` = c("32MB", "2.1MB", "236KB", "14×", "138×")
)

colnames(data)[1] <- ""
colnames(data)[2] <- "Storage Size"

kbl(data,
  booktabs = T,
  caption = "Comparison of storage size of functions.\\label{tab:function_sizes}",
  align = "lrrr",
) %>%
  kableExtra::kable_styling(
    # full_width = T,
...
Like our observations, they reported that Faaslets achieved cold start
...
Docker in regards to startup, runtime, and energy consumption. 
...
significant energy savings, the reduced storage required for storing functions,
and the low memory usage \citep{shillakerFaasmLightweightIsolation2020a}
achieved by \ac{Wasm} make it an attractive choice for resource-constrained
environments, such as edge computing devices or embedded systems. 
...
computing, particularly in scenarios involving small, repeated workloads. It is
important to note that our testing focused exclusively on this specific type of
workload. Therefore, while Wasm shows superiority in such situations, further
research is needed to evaluate its performance in other types of services.

Nevertheless, the findings presented in this thesis provide a solid foundation
for further exploration and adoption of Wasm in the serverless ecosystem,

...
\centering
  ...
  \caption{DevOps engineer deploying services as containers on Google Cloud
Platform (GCP).}
  \label{fig:docker}
\end{figure}
...
memory, storage, and network interfaces. Furthermore they describe that VMs run
...
\Cref{fig:vm-figure} below illustrates virtual machines running in such an
environment.
...
also introduce some challenges including, cold start latency and the lack of a
standardized approach to container execution across different platforms
\citep{missing}. 
...
strict performance requirements \citep{missing}. Additiionally, the lack of a
standardized approach to container execution means that developers may need to
create platform-specific configurations and optimizations, reducing the
portability of their functions across different FaaS providers \citep{missing}.

The limitations of container in FaaS environments have led to the exploration of
...
platform-agnostic approach to serverless computing, a potential is explored and
...
development of a more efficient and portable format \citep{wasmFaq}. 

...
stdout processes. 
2. Time the moment the container spawns, this will be compared against the time
   timed inside container, to get an accurate cold start time.
...
```{r, avg-times-nrec-rpi, echo=F, warning=F, dev = "cairo_pdf", fig.height=7, fig.align = 'center', fig.cap="Average Startup and Runtime on both our setups"}
...
ggplot(averaged_data, aes(x = func_name, y = avg_energy_consumption, fill = func_type)) +
  ...
  labs(x = "Function Name", y = "Average Energy Consumption (mWh)") +
  theme_minimal() +
  # theme_ipsum(base_family = "Avenir", base_size = 12, axis_text_size = 10, plot_title_size = 10, axis_title_size = 15, strip_text_size = 15) +
  theme(
...
showed startup times approximately 300-400 times faster than their Docker
...
starting up 240 times faster than Docker containers. In
...
is consistent. \todo{get these numbers}
...
On the virtual machine, \ac{Wasm} runtimes were between 100-200 times faster
than Docker runtimes, while on the Raspberry Pi, \ac{Wasm} still showed
significant runtime improvements compared to Docker containers.
...
1500 respectively.This indicates that the improved runtimes of
our \ac{Wasm}-based functions as an alternative to Docker diminishes at heavier
workloads.
...
approximately 200 times less energy \todo{get accurate figure}, measured in
microwatt-hours (μWh), than their Docker counterparts for the same computational
workload.
...
data <- data.frame(
  Average_Cold_Start = c(10, 12, 15),
  Average_Runtime = c(50, 60, 70)
)

# Create the table
ggplot(data) +
  geom_table(aes(x = "", y = Average_Cold_Start, label = paste("Average Cold Start: ", round(Average_Cold_Start), " seconds")),
    height = unit(2.5, "cm")
  ) +
  geom_text(
    aes(
      x = 0.3, y = max(data$Average_Cold_Start) + 1,
      label = "Cold Start (seconds)", hjust = 0, vjust = -0.5
    ),
    fontface = "bold"
  ) +
  theme_classic()
...
of whether or not our hypothesis that \ac{Wasm} offers a more efficient
alternative to Docker in regards for startup, runtime and energy consumption.

These findings underscore the potential of \ac{Wasm} as a lightweight and
efficient execution environment for serverless functions, offering substantial
performance benefits over traditional container-based approaches. The drastic
reduction in startup latency and overall runtime times can be particularly
advantageous in scenarios where minimizing latency and maximizing throughput are
critical, such as in latency-sensitive applications or high-throughput
workloads.

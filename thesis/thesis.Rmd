...
```{r, echo=F, warning=F}
ggplot(nrec_data, aes(x = input, y = total_runtime, color = func_type)) +
...
```{r, echo=F, warning=F}
power_rip_data <- rpi_data %>%
  filter(func_name == "factorial")

ggplot(power_rip_data, aes(x = input, y = average_power, color = func_type)) +
  geom_point(alpha = 0.41) + # Adjust transparency with alpha

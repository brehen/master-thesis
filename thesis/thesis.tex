...
The launch of AWS Amazon S3 cloud service in March 2006, followed by
...
values on a MQTT-topic. This is in turn picked up by the MQTT broker and
published to its subscribers. This switch supports reporting
...
with MQTT, is to utilize the Modbus TCP protocol. Modbus TCP is a widely
adopted industrial communication protocol for exchanging data between
devices and control systems. It has been used to develop energy
...
of interfaces, such as REST API, HTTPS, SNMP, Telnet, MQTT and Modbus
TCP \citep{gmbhExpertPowerControl2023}. In \Cref{fig:gude-control}
below, a simplified setup with the Gude acting as the server for
communicating over Modbus TCP is shown.
...
typical machine hosted on the cloud. For this, we will be using NREC
\todo{add citation/link}, a IaaS platform available to students, where
we can spin up a Debian virtual machine and set up a service on a public
IP address.
...
\item
  ...
  (e.g., NREC), to measure performance relative to how a typical cloud
  native application would perform.
\end{enumerate}
...
and the functions to a virtual machine running Debian on NREC. While a
Raspberry Pi 4 will serve as our deployment target for measuring startup
latencies and runtimes of a device more akin to embedded systems, but
more importantly, allowing us to measure energy consumption on a
controlled device.
...
their cloud platform as a way to offer a \ac{FaaS} powered by
\ac{WebAssembly} modules. They have built a cloud application platform
for \ac{Wasm}-based serverless functions named Fermyon Cloud, which is
...
traditional command-line programs. As \citet{malmgrenGettingDataOut2022}
suggested in their helpful article, using this feature of WASI provided
a simpler approach for handling input and output in our Wasm module
execution model. This approach was also implemented for passing input
and output to and from our Docker containers, and while using stdin and
stdout should incur some cost in terms of our metrics, this cost applies
to both deployment methods, ensuring a fair assessment.
...
target architecture we want our Docker container to run on. For our NREC
deployment, we will target linux/amd64 and for the Raspberry Pi, we will
target linux/arm64.
...
Once inside the NREC platform, we can launch an instance of our virtual
machine, following their guide\todo{add guide?} for doing so. For our
experiments we chose the pre-configured flavor of \texttt{m1.medium},
which gives us 1 virtual CPU, 4GB of RAM and 20GB storage space. The
virtual machine was built with the base image of Debian 12. After this
is set up, we can ssh into the server with a ssh-key we provided during
setup.
...
the NREC implementation, a Github action was implemented for building
the application on a Linux VM as part of their CI/CD service. This is
not a crucial detail for our research, but valuable for anyone who would
want to recreate the experiment and expand upon it.
...
our Raspberry Pi and virtual machine on NREC, we can start implementing
the benchmarking utility that will be used to conduct the experiments
and collect our data. While there exist tools out there for performing
experiments like this, the setup we have implemented would be difficult
to cover with an already existing tool. Because of this, we will develop
a custom benchmark utility in Rust, which will let us collect the data
we're interested in.
...
MQTT. This setup had one major flaw; energy reporting through MQTT could
only be reported once every 30 seconds at a minimum. This turned out to
be too slow for attempting to measure accurate readings for our
experiments.
...
network over ethernet, and changing the communcation method from MQTT to
Modbus TCP, we were able to perform energy readings in microseconds.
...
\begin{table}[H]
\centering
\caption{\label{tab:unnamed-chunk-2}The table is here \label{tab:dockervsfaaslet}}
\centering

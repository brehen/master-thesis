...
average showed startup times approximately 300-400 times faster than
their Docker counterparts on the virtual machine setup. While the gap is
smaller on Raspberry Pi, due to its resource constraints, it is still
noticeable with \ac{Wasm} starting up 240 times faster than Docker
containers. In \Cref{fig:nrec-efficiency, fig:rpi-efficiency} we can see
the detailed startup times of our functions across both our setups, and
while the values can vary a lot across invocations, we can see that the
trend for a consistent startup time is consistent.
\todo{get these numbers}
...
between 100-200 times faster than Docker runtimes, while on the
...
revealed that \ac{Wasm} function executions consumed approximately 200
times less energy \todo{get accurate figure}, measured in
microwatt-hours (μWh), than their Docker counterparts for the same
...
the topic of whether or not our hypothesis that \ac{Wasm} offers a more
efficient alternative to Docker in regards for startup, runtime and
energy consumption.

These findings underscore the potential of \ac{Wasm} as a lightweight
and efficient execution environment for serverless functions, offering
substantial performance benefits over traditional container-based
approaches. The drastic reduction in startup latency and overall runtime
times can be particularly advantageous in scenarios where minimizing
latency and maximizing throughput are critical, such as in
latency-sensitive applications or high-throughput workloads.

...
\titleformat{\part} {\headingfont\huge\bfseries\centering\vfill} {\partname\ \thepart} {12pt} {\centering} [\vfill]
...
\urlstyle{same}
\hypersetup{
  pdftitle={Nebula:~Performance and Energy Efficiency in Serverless Computing},
...
illustrated in \Cref{fig:wasm-wave}.
...
performance of \ac{Wasm} modules. \Cref{fig:wasm-runtimes} below
...
\citep{gmbhExpertPowerControl2023}. In \Cref{fig:gude-control} below, a
...
readings. \Cref{tab:gude_config} below shows a sample of sensors we can
...
As we can see from \Cref{tab:gude_config}, we can read \emph{Absolute
...
\Cref{chap:results}, the energy it takes to invoke a function as either
...
and Docker containers. As we can see in \Cref{tab:gude_config} there
...
our Docker containers as described in \Cref{sect:execute_docker},
...
maintaining its function. More on that in \Cref{sect:func_storage}.
...
table of sensor that it can report on. In \Cref{tab:gude_config}, we
...
\Cref{tab:nrec-avg,tab:rpi-avg}. These tables shows the direct
...
\Cref{tab:gude_config}, there is a small degree of inaccuracy in the
...
image and gzipped. \Cref{tab:function_sizes} shows the storage size,
